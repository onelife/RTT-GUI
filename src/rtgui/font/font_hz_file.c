...
static void rtgui_hz_file_font_draw_text(rtgui_font_t *font, rtgui_dc_t *dc,
    const char *text, rt_ubase_t len, rtgui_rect_t *rect);
static void rtgui_hz_file_font_get_metrics(rtgui_font_t *font, const char *text,
    rtgui_rect_t *rect);
...
/* Exported constants --------------------------------------------------------*/
const rtgui_font_engine_t rtgui_hz_file_font_engine = {
    RT_NULL,
...
/* Private functions ---------------------------------------------------------*/
/* Public functions ----------------------------------------------------------*/
static int _font_cache_compare(struct hz_cache *cache_1,
    struct hz_cache *cache_2) {
...
static rt_uint8_t *_font_cache_get(struct rtgui_hz_file_font *font,
    rt_uint16_t hz_id) {
    rt_uint32_t seek;
    ...
    cache = (struct hz_cache *)rtgui_malloc(
        sizeof(struct hz_cache) + font->font_data_size);
    if (!cache) return RT_NULL; /* no memory yet */
    ...
    if (font->cache_size >= HZ_CACHE_MAX) {
        ...
        while (SPLAY_LEFT(left, hz_node) != RT_NULL)
            left = SPLAY_LEFT(left, hz_node);
...
static void _rtgui_hz_file_font_draw_text(
    struct rtgui_hz_file_font *hz_file_font, rtgui_dc_t *dc, const char *text,
...
static void rtgui_hz_file_font_draw_text(rtgui_font_t *font, rtgui_dc_t *dc,
    ...
    str = rtgui_malloc(UTF_8ToGB2312_LEN((char *)text, length));
    if (str) {
        str_p = str;
    ...
    while (str_len > 0) {
        len = 0;
        while (((rt_uint8_t)*(str + len) < 0x80) && *(str + len) && \
            (len < str_len)) {
            len++;
        }
        ...
        if (len > 0) {
            _rtgui_hz_file_font_draw_text(
                hz_file_font, dc, (const char *)str, len, &text_rect);
            str += len;
            str_len -= len;
        }
    }
...
static void rtgui_hz_file_font_get_metrics(rtgui_font_t *font, const char *text,
    ...
    str = rtgui_malloc(UTF_8ToGB2312_LEN((char *)text, rt_strlen(text)));
    if (str)
        UTF_8ToGB2312((char *)str, (char *)text, rt_strlen(text));
    else
        return;
